...
def plot_fourier_series_and_function_in_custom_range(f, n, N, interval):
    """Plot the Fourier series approximation and the original function on a custom interval."""
    x_values = np.linspace(interval[0], interval[1], 1000)
    y_values_approx = [fourier_series_approximation(f, n, N, xi, (-np.pi, np.pi)) for xi in x_values]
    ...
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.title(f'Original Function and Fourier Series Approximation on {interval}')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def main():
    n = 3  # Replace with your student number
    N = 10  # Order of approximation
    interval = (-np.pi, np.pi)

    plot_fourier_series_and_function_in_custom_range(f, n, N, interval)
    ...
    custom_interval = (2, 3)
    plot_fourier_series_and_function_in_custom_range(f, n, N, custom_interval)


def plot_fourier_series_and_function_in_range(f, n, N, interval=(-np.pi, np.pi)):
